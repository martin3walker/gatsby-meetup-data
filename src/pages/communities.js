...
const OverviewCharts = ({ data }) => {
  ...
  const communityOverviewChart = {
    labels: data.allCommunityData.edges.map(edge => edge.node.city),
    datasets: [
      {
      ...
      },
      {
        ...
        label: 'Members with more than 1 RSVP',
      },
      {
        data: data.allCommunityData.edges.map(edge =>
          Math.round(edge.node.average_rsvps)
        ),
        borderWidth: 2,
